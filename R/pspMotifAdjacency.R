...
pspMotifAdjacency <- function(sp, ep, databasePSP, motif.ref, mdsite){
  ...
  # modified site is T or F
  if (mdsite == TRUE){
    # Duplicate and Add value=1
    rel_adj_pspmotif <- rel_adj_pspmotif

  }else{
    # Duplicate and Add value=1
    rel_adj_pspmotif <- suppressWarnings(rel_adj_pspmotif %>%
      separate(psite, into = c("psite", "residue"), sep = "_") %>%
      select(GENE, psite, value))
  }

                            ...
                            mdsite)
{
  try({
    ...
    rownames(input) <- gsub("-", ".", rownames(input))
    ...
    if (length(intersect_substrates) > 1){
      ...
      if (length(colnames(kinase_table_md)) > 1){
        ...
        summary_table <- quiet(summary(fit_kinase))
        summary_PE <- quiet(data.frame(summary_table[[5]]))
        ...
        ######
        activity_list$kinase <- kinase_table_md %*% abs(estimate)
